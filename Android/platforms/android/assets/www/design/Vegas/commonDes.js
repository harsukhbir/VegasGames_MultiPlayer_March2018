...
_h8._wc._an = {
  init: function () {
    ...
    this.REGISTRATION_INPUT = '<div class="row-form sign-up-row-form">\t\t\t<div class="input-row-form sign-up-input-row-form" >\t\t\t<input type="{type}"  name="{name}" id="{id}" {required} pattern="{pattern}" data-bind="click: CustomKeyboard.getTypeInput.bind($data,`{id}`)" value="{value}" class="input-white {required}">\t\t\t</div>\t\t\t</div>';
